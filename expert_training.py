...
# ─────────────────────────────────────────────────────────────────
# STEP 2a-SAC: CartPole-v1 with SAC + ContinuousCartPoleWrapper
#
# WHY SAC FOR CARTPOLE?
#   ML-IRL and IL-SOAR are built on SAC, which requires a continuous
#   action space.  ContinuousCartPoleWrapper exposes Box([-1,1]) and
#   internally maps the sign of the action to push-left / push-right,
#   so the underlying dynamics and reward are unchanged.
# ─────────────────────────────────────────────────────────────────

def train_cartpole_sac(total_timesteps=100_000):
    print("\n" + "=" * 60)
    print("  STEP 2a-SAC: CartPole-v1  (SAC + continuous wrapper)")
    print("=" * 60)
    print("  Obs:  [cart_pos, cart_vel, pole_angle, pole_vel]")
    print("  Act:  continuous a in [-1,1]  (a<0 -> left, a>=0 -> right)")
    print("  Goal: keep pole upright for 500 steps  ->  max return = 500")

    train_env = ContinuousCartPoleWrapper(gym.make("CartPole-v1"))
    eval_env  = ContinuousCartPoleWrapper(gym.make("CartPole-v1"))
    ...
    mean_r, std_r = evaluate_policy(
        model, eval_env, n_eval_episodes=50, deterministic=True, warn=False
    )
    print(f"\n  Final expert return: {mean_r:.1f} +/- {std_r:.1f}  (optimal = 500)")

    model.save("models/expert_cartpole_sac")
    print("  Saved -> models/expert_cartpole_sac.zip")
...
def plot_all(results: dict):
    ...
    configs = {
        ...
        "cartpole_sac": dict(
            title="CartPole-v1  (SAC + continuous wrapper)",
            color="#673AB7",
            ref_y=500, ref_label="Optimal (500)",
            ylabel="Mean Episode Return",
            ylim=(0, 550),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env",
        choices=["cartpole", "cartpole_sac", "pendulum", "mcc", "all"],
        default="all",
        help="Which env to train",
    )
    parser.add_argument("--steps_cartpole",     type=int, default=100_000)
    parser.add_argument("--steps_cartpole_sac", type=int, default=100_000)
    ...
    if args.env in ("cartpole_sac", "all"):
        results["cartpole_sac"] = train_cartpole_sac(args.steps_cartpole_sac)
    ...
    print("=" * 60)
    labels = {
        "cartpole":     "CartPole (PPO)",
        "cartpole_sac": "CartPole (SAC + wrapper)",
        "pendulum":     "Pendulum (SAC)",
        "mcc":          "MCC (TD3+OU)",
    }
    for key, (_, _, _, final) in results.items():
        print(f"  {labels[key]:<30} -> final return {final:.1f}")
